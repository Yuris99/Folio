...
(async()=>{
  try {
    ...
    const created=await json('/api/v1/applications',{method:'POST',body:JSON.stringify({company:'테스트 회사',role:'개발자',status:'작성 중',considering:true,deadline:'2026-09-01',next:'지원서 작성',applicationFitScore:99,compensationScore:99,companyScore:99})},cookie);
    ...
    const archivedWorkspace=(await json('/api/v1/bootstrap',{},cookie)).data.data;
    assert.equal(archivedWorkspace.applications.some(item=>item.id===applicationId),false);
    assert.ok(archivedWorkspace.jobs.some(item=>item.id===linkedJob.id));
    const archivedApplication=archivedWorkspace.archivedApplications.find(item=>item.id===applicationId);
    assert.equal(archivedApplication.status,completedApplication.status);
    assert.deepEqual(archivedApplication.processSteps,completedApplication.processSteps);
    assert.equal(archivedApplication.applicationFitScore,25);
    assert.equal((await json('/api/v1/account/export',{},cookie)).data.data.workspace.archivedApplications[0].id,applicationId);
    const restoreUrl='/api/v1/applications/'+applicationId+'/restore';
    ...
    assert.equal(restoredWorkspace.archivedApplications.length,0);
    assert.equal((await json(restoreUrl,{method:'POST'},cookie)).response.status,404);
    await json('/api/v1/applications/'+applicationId,{method:'DELETE'},cookie);
    const replacement=await json('/api/v1/applications',{method:'POST',body:JSON.stringify({jobId:linkedJob.id})},cookie);
    assert.equal((await json(restoreUrl,{method:'POST'},cookie)).response.status,409);
    const conflictWorkspace=(await json('/api/v1/bootstrap',{},cookie)).data.data;
    assert.ok(conflictWorkspace.archivedApplications.some(item=>item.id===applicationId));
    assert.ok(conflictWorkspace.applications.some(item=>item.id===replacement.data.data.id));
