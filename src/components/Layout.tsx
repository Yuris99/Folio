    ...
    <div className={`app-shell view-${view}`}>
      <aside className="sidebar">
        ...
        <nav className="nav" aria-label="주요 메뉴">
          {navItems.map((item) => <button key={item.view} className={`nav-item ${view === item.view ? 'active' : ''}`} onClick={() => move(item.view)}><span>{item.icon}</span>{item.label}</button>)}
        </nav>
