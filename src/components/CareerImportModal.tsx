...
type ImportKind = 'career' | 'applications' | 'interviews' | 'documents' | 'tasks';
type ChatImport = { format: 'folio-chat-import'; version: 1; kind: Exclude<ImportKind, 'career'>; data: Record<string, unknown[]> };
type CareerImport = Record<string, unknown> & { format: 'folio-career-import'; version: 1 };
...
const prompts: Record<ImportKind, { label: string; description: string; prompt: string }> = {
...
function parse(text: string, selected: ImportKind): CareerImport | ChatImport {
  ...
  if (value.format !== 'folio-chat-import' || value.kind !== selected || !value.data || typeof value.data !== 'object') throw new Error(`${prompts[selected].label}용 JSON 형식이 아닙니다.`);
...
export function CareerImportModal({ mutate, onClose, onImported }: { mutate: Mutation; onClose: () => void; onImported: (kind: ImportKind) => void }) {
  ...
  const validation = useMemo(() => { if (!text.trim()) return { value: null, error: '' }; try { return { value: parse(text, kind), error: '' }; } catch (error) { return { value: null, error: error instanceof Error ? error.message : 'JSON을 확인해 주세요.' }; } }, [text, kind]);
  ...
  async function copyPrompt() { await navigator.clipboard.writeText(prompts[kind].prompt); window.alert(`${prompts[kind].label} 정리용 프롬프트를 복사했습니다.`); }
  ...
  async function submit() {
    ...
    const result = validation.value.format === 'folio-career-import'
      ...
      : await mutate(`${prompts[kind].label} 가져오기`, () => api.importChatData(validation.value));
  ...
  return <Modal title="기존 AI 채팅에서 가져오기" kicker="NO API TOKEN" onClose={onClose}>
    <div className="import-kind-grid">{(Object.keys(prompts) as ImportKind[]).map((item) => <button type="button" key={item} className={kind === item ? 'active' : ''} onClick={() => selectKind(item)}><b>{prompts[item].label}</b><small>{prompts[item].description}</small></button>)}</div>
    <div className="career-import-flow">
      <section><span className="import-step">1</span><div><h3>{prompts[kind].label} 정리 요청하기</h3><p>프롬프트를 기존 채팅에 붙여넣으세요. Folio의 AI API나 토큰은 사용하지 않습니다.</p><button type="button" className="button" onClick={() => void copyPrompt()}>이 프롬프트 복사</button></div></section>
