...
export function JobCreateModal({ mutate, onClose, onCreated }: { mutate: Mutation; onClose: () => void; onCreated: (job: Job) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const base = { company: String(data.get('company')), role: String(data.get('role')), location: String(data.get('location')), deadline: alwaysOpen ? '' : String(data.get('deadline')), alwaysOpen, url: String(data.get('url')), notionUrl: String(data.get('notionUrl') || '').trim(), description: String(data.get('description')) };
