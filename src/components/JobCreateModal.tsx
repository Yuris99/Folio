...
export function JobCreateModal({ mutate, onClose, onCreated }: { mutate: Mutation; onClose: () => void; onCreated: (job: Job) => void }) {
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const base = { company: String(data.get('company')), role: String(data.get('role')), location: String(data.get('location')), deadline: String(data.get('deadline')), url: String(data.get('url')), description: String(data.get('description')) };
