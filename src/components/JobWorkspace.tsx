...
export function JobWorkspace({ job, attachments, mutate, onBack, onCreateApplication }: { job: Job; attachments: Attachment[]; mutate: Mutation; onBack: () => void; onCreateApplication?: () => void }) {
  ...
  return <div className="job-workspace">
    ...
    <main className="job-page-editor">
      ...
      {activeId === 'root' && <div className="job-root-actions"><a className="button" href={job.url} target="_blank" rel="noreferrer">원본 공고 열기</a>{job.notionUrl ? <a className="button" href={job.notionUrl} target="_blank" rel="noreferrer">노션 바로가기 ↗</a> : <button className="button" disabled title="공고 정보에서 노션 URL을 입력하고 저장하세요">노션 링크 미등록</button>}{onCreateApplication && <button className="button primary" onClick={onCreateApplication}>지원 건 만들기</button>}</div>}
    </main>
  </div>;
}
