...
export function JobWorkspace({ job, mutate, onBack, onCreateApplication }: { job: Job; mutate: Mutation; onBack: () => void; onCreateApplication: () => void }) {
  ...
  return <div className="job-workspace">
    ...
    <main className="job-page-editor">
      ...
      {preview ? <Markdown source={content} /> : <textarea ref={editorRef} className="job-markdown-editor" value={content} onChange={(event) => changeContent(event.target.value)} placeholder="# 제목\n\n위 서식 버튼을 사용하거나 Markdown으로 자유롭게 정리하세요." />}
