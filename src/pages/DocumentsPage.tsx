...
export function DocumentsPage({ workspace, navigate, mutate }: { workspace: Workspace; navigate: (view: View) => void; mutate: Mutation }) {
  ...
  return <>
    ...
    </div> : <EmptyState title="아직 지원 문서가 없습니다." description="회사별 문서를 만들거나 저장한 공고에서 AI 초안을 생성할 수 있습니다." action={<button className="button primary" onClick={() => setCreating(true)}>첫 문서 만들기</button>} />}
