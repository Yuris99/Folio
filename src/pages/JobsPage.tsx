...
export function JobsPage({ workspace, navigate, mutate }: { workspace: Workspace; navigate: (view: View) => void; mutate: Mutation }) {
  ...
  return <>
    ...
    {creating && <Modal title="채용 공고 저장" kicker="NEW OPPORTUNITY" onClose={() => setCreating(false)}><form onSubmit={create}><p className="muted">공고 본문을 붙여 넣으면 핵심 기술을 분석해 저장합니다.</p><div className="form-grid two"><label>회사명<input required name="company" /></label><label>직무명<input required name="role" /></label></div><div className="form-grid two"><label>마감 일시<input name="deadline" type="datetime-local" min="2000-01-01T00:00" max="2100-12-31T23:59" /></label><label>공고 URL<input name="url" type="url" placeholder="https://..." /></label></div><label>채용 공고 본문<textarea required name="description" rows={10} placeholder="주요 업무, 자격 요건, 우대 사항을 붙여 넣으세요." /></label><div className="modal-actions"><button type="button" className="button ghost" onClick={() => setCreating(false)}>취소</button><button className="button primary">분석하고 저장</button></div></form></Modal>}
  </>;
}
