...
export function ApplicationsPage({ workspace, navigate, mutate }: { workspace: Workspace; navigate: (view: View) => void; mutate: Mutation }) {
  ...
  const [sortBy, setSortBy] = useState<'recent' | 'priority' | 'deadline' | 'company'>('recent');
  ...
  const visibleApplications = useMemo(() => workspace.applications.filter((application) => {
    ...
    if (sortBy === 'priority') return ({ 높음: 0, 보통: 1, 낮음: 2 }[a.priority || '보통']) - ({ 높음: 0, 보통: 1, 낮음: 2 }[b.priority || '보통']);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: ApplicationPayload = {
      company: String(data.get('company')), role: String(data.get('role')), location: String(data.get('location')), status: String(data.get('status')), priority: String(data.get('priority')) as ApplicationPayload['priority'],
  ...
  return <>
    ...
    <div className="application-toolbar">
      ...
      <select value={sortBy} onChange={(event) => setSortBy(event.target.value as 'recent' | 'priority' | 'deadline' | 'company')} aria-label="지원 정렬"><option value="recent">최근 수정순</option><option value="priority">우선순위순</option><option value="deadline">마감 임박순</option><option value="company">회사명순</option></select>
    ...
    <div className="application-list">
      {visibleApplications.map((application) => {
        ...
        return <article className="application-row" key={application.id}>
          ...
          <div className="app-status-stack"><span className={`status status-${statusClass(application.status)}`}>{normalizedApplicationStatus(application.status)}</span><span className={`priority priority-${application.priority === '높음' ? 'high' : application.priority === '낮음' ? 'low' : 'normal'}`}>{application.priority || '보통'} 우선</span></div>
    ...
    {modalOpen && <Modal title={editing ? '지원 수정' : '지원 추가'} kicker="APPLICATION" onClose={() => setModalOpen(false)}><form key={editingId || 'new'} onSubmit={submit}>
      ...
      <div className="form-grid two"><label>현재 상태<select name="status" defaultValue={normalizedApplicationStatus(editing?.status || '관심')}>{applicationStatuses.map((status) => <option key={status}>{status}</option>)}</select></label><label>우선순위<select name="priority" defaultValue={editing?.priority || '보통'}><option>높음</option><option>보통</option><option>낮음</option></select></label></div>
