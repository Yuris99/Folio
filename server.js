...
async function api(req,res,url) {
  ...
  if(jobMatch&&method==='PATCH'){const item=w.jobs.find(x=>x.id===jobMatch[1]);if(!item)return fail(res,404,'공고를 찾을 수 없습니다.','NOT_FOUND');for(const key of ['company','role','deadline','url','description','skills','companyAnalysis','pageContent','pageHtml','coverImage','pages','attachmentIds'])if(payload[key]!==undefined)item[key]=payload[key];item.updatedAt=now();saveDb();return ok(res,item);}
