...
function scheduleWorkspace(workspace){
  const rejected=status=>['불합격','탈락'].includes(status);
  const rejectedIds=new Set([...(workspace.applications||[]),...(workspace.archivedApplications||[])].filter(item=>rejected(item.status)).map(item=>item.jobId));
...
async function api(req,res,url) {
  ...
  if(method==='POST'&&route==='/api/v1/jobs'){const item={...payload,id:uid(),alwaysOpen:Boolean(payload.alwaysOpen),deadline:payload.alwaysOpen?'':String(payload.deadline||''),createdAt:now()};w.jobs.unshift(item);saveDb();return ok(res,item,201);}
  const jobMatch=route.match(/^\/api\/v1\/jobs\/([^/]+)$/);
  if(jobMatch&&method==='PATCH'){const item=w.jobs.find(x=>x.id===jobMatch[1]);if(!item)return fail(res,404,'공고를 찾을 수 없습니다.','NOT_FOUND');for(const key of ['company','role','location','deadline','alwaysOpen','url','notionUrl','description','skills','companyAnalysis','pageContent','pageHtml','coverImage','pages','attachmentIds'])if(payload[key]!==undefined)item[key]=payload[key];if(item.alwaysOpen)item.deadline='';item.updatedAt=now();saveDb();return ok(res,item);}
  if(method==='POST'&&route==='/api/v1/applications'){const same=(a,b)=>String(a||'').trim().replace(/\s+/g,' ').toLocaleLowerCase()===String(b||'').trim().replace(/\s+/g,' ').toLocaleLowerCase(),score=(value,min,max)=>Math.min(max,Math.max(min,Number(value)||0));let job=w.jobs.find(j=>j.id===payload.jobId)||w.jobs.find(j=>same(j.company,payload.company)&&same(j.role,payload.role));if(!job){job={id:uid(),company:String(payload.company||'').trim(),role:String(payload.role||'').trim(),location:payload.location||'',deadline:payload.alwaysOpen?'':payload.deadline||'',alwaysOpen:Boolean(payload.alwaysOpen),url:payload.url||'',notionUrl:payload.notionUrl||'',description:'',skills:[]};w.jobs.unshift(job)}else for(const key of ['location','deadline','alwaysOpen','url','notionUrl'])if(payload[key]!==undefined)job[key]=payload[key];if(job.alwaysOpen)job.deadline='';const item={id:uid(),jobId:job.id,status:payload.status||'관심',considering:Boolean(payload.considering),careerGrade:['S','A','B','C','D'].includes(payload.careerGrade)?payload.careerGrade:undefined,applicationFitScore:score(payload.applicationFitScore,0,25),compensationScore:score(payload.compensationScore,0,15),companyScore:score(payload.companyScore,0,5),locationScore:score(payload.locationScore,0,10),processScore:score(payload.processScore,0,10),priorityAdjustment:score(payload.priorityAdjustment,-10,10),appliedAt:payload.appliedAt||'',nextProcess:payload.nextProcess||payload.next||'',nextDate:payload.nextDate||'',processSteps:Array.isArray(payload.processSteps)?payload.processSteps:[],next:payload.nextProcess||payload.next||'',memo:payload.memo||'',rejectionReason:String(payload.rejectionReason||''),createdAt:now()};w.applications.unshift(item);saveDb();return ok(res,item,201);}
  ...
  if(method==='POST'&&route==='/api/v1/files'){
    ...
    const detectedType=buffer.subarray(0,5).toString()==='%PDF-'?'application/pdf':buffer.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10]))?'image/png':buffer[0]===255&&buffer[1]===216?'image/jpeg':buffer.subarray(0,3).toString()==='GIF'?'image/gif':buffer.subarray(0,4).toString()==='RIFF'&&buffer.subarray(8,12).toString()==='WEBP'?'image/webp':buffer.subarray(0,2).toString()==='BM'?'image/bmp':'';
